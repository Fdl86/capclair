...
function recordedOrDerivedSpeed(
  ...
  return allowDerived ? derivedSpeedKt(previous, current, sameSegment) : null;
}
...
export function buildReplayModel(trace: Trace): ReplayModel {
  const temporalAvailable = trace.timingMode !== 'unavailable';
  ...
  valid.forEach(({ position, originalIndex }, validIndex) => {
    ...
    if (breakBefore) {
      segmentIndex += 1;
      currentSegment = createSegment(segmentIndex, validIndex, activeTimeMs, temporalAvailable ? Math.max(0, deltaMs) : 0);
      segments.push(currentSegment);
    ...
    const speedKt = recordedOrDerivedSpeed(previousPosition, position, sameSegment, temporalAvailable);
