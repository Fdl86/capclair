...
export async function analyzePibText(input: {
  ...
  if (notams.length === 0) throw new Error('Aucun NOTAM structuré n’a été détecté dans ce contenu. Vérifiez qu’il s’agit bien d’un briefing SOFIA textuel.');
  const references = uniqueReferences(notams.flatMap((notam) => notam.supAipReferences));
  const reconciliations = await reconcileSupAipReferences(references);
