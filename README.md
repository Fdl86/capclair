# CAP CLAIR WEB13.30.2 - NOTAM PIB BETA
...
## WEB13.30.2 - NOTAM MAP UX HOTFIX


- correction du copier-coller SOFIA : les rubriques vides et blocs `NIL` ne sont plus rattachés au NOTAM précédent ;
- fiche NOTAM cartographique avec en-tête fixe, fermeture toujours accessible et contenu interne défilant ;
- touche `Échap` disponible pour fermer la fiche sur ordinateur ;
- désencombrement de la carte : les libellés `Approximation Q` restent visibles pour les créneaux actifs, tous les autres cercles Q restent présents et cliquables sans répétition du libellé ;
- aucune refonte des sections ou de la navigation dans ce hotfix.
...
Cette livraison est un zip complet WEB13.30.2.
...
4. Copier tout le contenu du zip WEB13.30.2 à la racine du dépôt.
5. Dans GitHub Desktop, vérifier les changements, créer le commit puis utiliser `Push origin` sur `main`.
6. Après le déploiement Cloudflare Pages, vérifier `WEB13.30.2` dans la chip visible et dans le titre de l'onglet.
